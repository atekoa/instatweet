    ...
    $(this).each(function () {
      ...
      if (this.tweetMachine) {
        ...
        settings = $.extend({
          ...
          user_name:		'TWITTER_USER_NAME', // Set your username
          include_retweets: false, // Set to true or false if you want to include retweets
          exclude_replies: true, // Set to true or false if you want to exclude replies
          rate: 15000, // Rate in ms to refresh the tweets. Any higher than 5000 for search/tweets will get you rate limited
          limit: 100, // Number of tweets to display at a time
          ...
          tweetFormat: "<div class='tweet'><h3 class='fullname'>ESTE EL USUARIO</h3><p class='text'>AQUI EL TEXTO</p><a target='_blank' class='link' href=''><img src='' class='image'></a></div>", // Format for each tweet
          ...
          filter: function(tweet) { // Function to filter tweet results.
        ...
        this.tweetMachine = {
          ...
          */
          ...
          buildTweet: function (tweet) {
            texto = "-";
            if(tweet.text != null) texto = tweetMachine.parseText(tweet.text + "<br>");
            lines = 20 * (tweet.text.length / 40);       
            if(tweet.entities.media!=null) {
              var $newEls = $('<div>').addClass('photo')
                ...
                .append($('<h3>').addClass('fullname')
                  .append($('<img>').attr('src', 'inc/twitter32.png').attr('style', 'height:15px;padding-left:5px;padding-right:5px;'))
                  .append($('<a>').attr('target', '_blank').attr('href', tweet.entities.media[0].expanded_url).html(tweet.user.name + "&nbsp;&diams;&nbsp;" + tweet.retweet_count))
                )
                // .append($('<p>').addClass('text').html(texto))
                .append($('<a>').attr('target', '_blank').addClass('image').attr('href', tweet.entities.media[0].media_url).attr('rel', tweet.entities.media[0].media_url)
                  .append($('<img>').addClass('instagram-image').attr('src', tweet.entities.media[0].media_url).attr('width', '306px').attr('height', eval(306*tweet.entities.media[0].sizes.small.h/tweet.entities.media[0].sizes.small.w))
                    .attr('style', 'z-index:-999; width: 306px; height:' + eval(306*tweet.entities.media[0].sizes.small.h/tweet.entities.media[0].sizes.small.w) +'px')
                  )
                );
            } else {
              var $newEls = $('<div>').addClass('photo')
                ...
                .append($('<h3>').addClass('fullname')
                  .append($('<img>').attr('src', 'inc/twitter32.png').attr('style', 'height:15px;padding-left:5px;padding-right:5px;'))
                  .append($('<a>').attr('target', '_blank').attr('href', 'http://twitter.com/' + tweet.user.screen_name + '/status/' + tweet.id_str).html(tweet.user.name + "&nbsp;&diams;&nbsp;" + tweet.retweet_count))
                )
                .append($('<p>').addClass('text').html(texto)
                );
            }
            
            $(tweetMachine.container).prepend( $newEls )
                .isotope('reloadItems')
                .isotope({ sortBy: 'original-order' }) // set sort back to symbol for inserting
